...
void Pathname::concat(const std::string& filename)
{
...
{
    return extra + STL_util::hash(path) + std::hash<std::string>()(base) + std::hash<std::string>()(ext);
}
...
std::string Pathname::to_platform_string() const
{
#ifdef PLATFORM_WIN
...
std::string Pathname::to_windows_string() const
{
    return to_string(windows_separator);
}

std::string Pathname::to_unix_string() const
{
    return to_string(unix_separator);
}

std::string Pathname::to_string(char sep) const
{
...
Pathname* Pathname::from_string(const std::string& filepath)
{
    return from_unix_string(filepath);
}

Pathname* Pathname::from_platform_string(const std::string& filepath)
{
#ifdef PLATFORM_WIN
...
Pathname* Pathname::from_windows_string(const std::string& filepath)
{
...
Pathname* Pathname::from_unix_string(const std::string& filepath)
{
    Pathname* result = new Pathname();
...
void Pathname::parse_windows_path(const std::string& filepath, String_vector& path, std::string& base, std::string& ext, byte& extra)
{
    size_t head;
...
void Pathname::parse_path(const std::string& filepath, size_t head, String_vector& path, std::string& base, std::string& ext, byte& extra)
{
    ...
    if (sep_pos == std::string::npos) {
        ...
        while ((sep_pos = filepath.find_first_of(any_separator, head)) < dir_pos) {
            std::string element = filepath.substr(head, sep_pos - head);
            if (!element.empty())
                path.append(element);
            head = sep_pos + 1;
        }
...
void Pathname::parse_windows_path(const std::string& filepath, char& drive, String_vector& path, std::string& base, std::string& ext, bool& is_abs, bool& is_dir)
{
    byte extra = 0;
...
void Pathname::parse_unix_path(const std::string& filepath, String_vector& path, std::string& base, std::string& ext, bool& is_abs, bool& is_dir)
{
    byte extra = 0;
